...
len_unfx = 4
g1_std = np.linspace(0.0, 0.3, len_unfx)  # as a fraction of KCD
k_std = np.linspace(0.0, 0.3, len_unfx)  # as a fraction of K
num_celltype = 2
...
if __name__ == '__main__':
    ...
    for i0 in xrange(start, stop):  # varying the different growth conditions
        ...
        temp3 = data[i0, 28, :]  # mean bud length -- td version
        temp4 = data[i0, 33, :]  # Whi5 daughter fraction f.
    ...
    if rank == 0:
        ...
        # plt.imshow(new_grid)
        # plt.show()
        np.save('Feb17_test_models3_model'+str(par1['modeltype'])+'_V2', new_grid)
