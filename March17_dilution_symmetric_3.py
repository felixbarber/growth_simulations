...
par_vals = dict([('num_s1', 500), ('nstep', 500), ('num_gen', 9), ('modeltype', 24), ('dt', 0.01), ('td', td),
...
save_freq = 500
num_cells = 3
# timer = 1.7  # for 8 cores
timer = 0.97  # for 4 cores
if __name__ == '__main__':
    ...
    for i0 in xrange(start, stop):  # variable number of repeats for each core
        ...
        for i1 in range(X[1]):
            ...
            if np.mod(i1, save_freq) == 0:
                np.save(
                    '../../Documents/data_storage/March17_dilution_symmetric_3_savedpop_model_{0}_rep_{1}_it_{2}_asymm'.format(str(par_vals['modeltype']),
                                                                                            str(i0), str(i1)), temp1)
    ...
    if rank == 0:
        ...
        # plt.imshow(new_grid)
        # plt.show()
        np.save('./March17_dilution_symmetric_3_model_{0}_asymm'.format(par_vals['modeltype']), new_grid)

# 50001 pts, 4 rep, 4 cores for 47052 seconds.
# v1 is done after the code is implemented to make it so that cells will never shrink or lose inhibitor during G2. Was
# done with 8 repetitions. Time taken: 85610 seconds.

# model 24 is for no negative growth at all, and includes the full population