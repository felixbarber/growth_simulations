...
par_set = dict([('g1_std', 0.0), ('dt', 0.01), ('td', 1.0), ('g1_delay', 0.0), ('num_s1', 500), ('nstep', 500),
                ('num_gen', 8), ('modeltype', 22), ('delta', delta), ('d_std', 0.0)])

model_descr = ['Noisy synth const frac no neg growth']

num_med = 5
num_rep = 2
len_var = 2  # number of sampling points for each variable
len_unfx = 4
g1_std = np.linspace(0.0, 0.1, 2)  # as a fraction of KCD
k_std = np.linspace(0.0, 0.2, 2)  # as a fraction of K
# k_std = np.array([0.5])
num_celltype = 2
...
if __name__ == '__main__':
    ...
    for i0 in xrange(start, stop):  # varying the different growth conditions
        ...
        temp2 = data[i0, 31, :]  # noise in bud length -- td version
        # temp3 = data[i0, 30, :]  # mean bud length -- gr version
        temp3 = data[i0, 32, :]  # mean bud length -- td version
    ...
    if rank == 0:
        ...
        # plt.imshow(new_grid)
        # plt.show()
        np.save('March17_test_models1_model'+str(par1['modeltype'])+'_V0', new_grid)
