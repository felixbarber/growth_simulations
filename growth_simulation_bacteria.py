...
class Cell(object):
    ...
    CD = 0.75*td  # time delay C+D after the initiation is begun
    ...
    def grow(self, par1):  # integration model for accumulation of initiator protein. In slow growth case.
        ...
        self.vi = ((self.delta+self.noise_thr-self.wb)/self.K+self.vb)*np.exp(self.noise_g1*np.log(2)/self.td)
        ...
        self.wd = self.K * (self.vd-self.vi)
...
def next_gen(index, f, time, par1):
    ...
    f.append(Cell(0.5*f[index].vd, 0.5*f[index].wd, time, mother=weakref.proxy(f[index])))
