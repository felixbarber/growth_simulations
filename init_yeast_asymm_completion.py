...
import os
import cluster_initiation_source as g
import time
...
hard_erase = 0  # If this is true, previous files will all be completely overwritten.
size = 200  # starts from 1.
rank = int(os.environ["PARAM1"]) - 1  # starts from zero. Imports the index tracking the number of iterations.
# Note that for this version, this should only be the number of jobs which did not go to completion.

# ------------------------------------------------  specific to individual scripts
ids = [1, 4, 95, 198]
rank = ids[rank]  # this ensures that the correct rank isin
path1 = './data/init_yeast_asymm_{0}.npy'.format(rank)  # path where the calculated values will be saved
path2 = './data/prog_{0}.npy'.format(rank)  # path where progress made is recorded.


delta = 10.0

par1 = dict([('dt', 0.01), ('td', 1.0), ('num_s1', 500), ('nstep', 500), ('num_gen', 10), ('modeltype', 4),
...
pars = [r, r_std, g1_thresh_std]
vals = ['r', 'r_std', 'g1_thresh_std']
...
for index in xrange(start, stop):
    ...
    c.append(temp1)
    del temp1, temp
    for i5 in range(num_celltype):  # celltype
        for i6 in range(num_sims):  # sim number
            ...
            a[temp_v1[0], temp_v1[1], temp_v1[2], temp_v1[3], i5, i6, 0] = val1[0]  # slope result
            a[temp_v1[0], temp_v1[1], temp_v1[2], temp_v1[3], i5, i6, 1] = len(x2)*100.0/len(x1)  # % with low div conc result
            a[temp_v1[0], temp_v1[1], temp_v1[2], temp_v1[3], i5, i6, 2] = len(x3) * 100.0 / len(x1)
...
# Time taken for 80 repeats: 778s. ~ 15s per run.
